...
// Get API key and List ID from config
...
if (isset($response['status'])) {
    if ($response['status'] == 'subscribed') {
        return redirect()->back()->with('info', 'This email is already subscribed to our newsletter.Thank You!');
    } elseif ($response['status'] == 'pending') {
        return redirect()->back()->with('info', 'This email is pending confirmation. Please check your inbox.');
    }
}

// 🔄 **Step 2: Try subscribing the email**
...
if ($mailchimp->success()) {
    ...
    // If MailChimp returns an error, show a meaningful message
    $errorMessage = $response['detail'] ?? 'Subscription failed. Please try again.';
